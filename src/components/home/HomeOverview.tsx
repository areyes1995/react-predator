...
import { ALL_MODULE_DATA, getAllModuleRecords, RECORD_STATUSES, STATUS_META, MODULE_BAR_COLORS } from '../../records'
    ...
    <div className="flex-1 overflow-auto px-4 lg:px-6 pt-5 pb-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {ALL_MODULE_DATA.map(entry => {
          ...
          const barClass = MODULE_BAR_COLORS[entry.color] ?? 'bg-blue-500'
            ...
            <div key={entry.label} className="bg-[var(--bg-surface-soft)] border border-[var(--border)] rounded-xl p-5 transition-all duration-300 hover:scale-[1.02] hover:bg-[var(--bg-surface-hover)] hover:border-[var(--border-active)]">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className={`w-2.5 h-2.5 rounded-full ${barClass}`} />
              ...
              <div className="h-2 bg-[var(--bg-surface)] rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barClass}`} style={{ width: `${entry.data.length / stats.total * 100}%` }} />
              </div>
              {sampleTitle && (
